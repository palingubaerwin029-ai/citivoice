  ...
  if (submitted) {
    return (
      <View style={[styles.successContainer, { backgroundColor: colors.bgDark }]}>
        ...
        <TouchableOpacity style={styles.successSecBtn} onPress={() => { 
            setSubmitted(false); 
            setCurrentStep(1);
          ...
          <React.Fragment key={step}>
            <View style={{ alignItems: 'center', flex: 1 }}>
              ...
              <Text style={[
                ...
                { color: currentStep >= step ? colors.textPrimary : colors.textMuted, fontWeight: currentStep === step ? '700' : '500' }
              ]}>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bgDark }} edges={['top', 'bottom']}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        ...
        <View style={[styles.customHeader, { backgroundColor: colors.bgCard, borderBottomColor: colors.border }]}>
          ...
          <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>{t('reportConcern')}</Text>
        ...
        {currentStep === 1 && (
          <View style={styles.stepContainer}>
            <View style={styles.stepHeaderRow}>
              <Text style={[styles.stepHeader, { color: colors.textPrimary, marginBottom: 0 }]}>Tell us more</Text>
            </View>
            <Text style={[styles.stepSub, { color: colors.textSecondary }]}>Provide a title and describe the details</Text>
            ...
            <PrimaryButton title="Next Step →" onPress={nextStep} style={{ marginTop: verticalScale(24) }} />
          </View>
        )}
        ...
        {currentStep === 2 && (
          <View style={styles.stepContainer}>
            <View style={styles.stepHeaderRow}>
              ...
              <Text style={[styles.stepHeader, { color: colors.textPrimary, marginBottom: 0 }]}>Add evidence</Text>
              <View style={{ width: 24 }} />
            </View>
            <Text style={[styles.stepSub, { color: colors.textSecondary }]}>Photos and location help us resolve issues faster</Text>

            {/* Photo */}
            <View style={styles.fieldGroup}>
              <Text style={[styles.stepHeader, { color: colors.textPrimary, fontSize: rf(16), marginBottom: verticalScale(12) }]}>
                📸 Photo <Text style={{ color: colors.statusRejected, fontWeight: '700' }}>*</Text>
              </Text>
            ...
            <View style={styles.fieldGroup}>
              <Text style={[styles.stepHeader, { color: colors.textPrimary, fontSize: rf(16), marginBottom: verticalScale(12) }]}>
                📍 Location <Text style={{ color: colors.statusRejected, fontWeight: '700' }}>*</Text>
              </Text>
              ...
              {showMap && (
                <View style={styles.mapSection}>
                  <View style={styles.mapContainer}>
                    ...
                    <View style={styles.mapHintContainer} pointerEvents="none">
                      <View style={[styles.mapHintBadge, { backgroundColor: colors.bgCard, borderColor: colors.border }]}>
                        ...
                        <Text style={[styles.mapHintText, { color: colors.textPrimary }]}>{t('tapToUseLocation')}</Text>
                      </View>
                    </View>
                  ...
                  <View style={[styles.addressCard, { backgroundColor: colors.bgCard, borderColor: colors.accent + '44', backgroundColor: colors.accent + '0A' }]}>
                    ...
                    <TouchableOpacity onPress={() => { setLocation(null); setShowMap(false); setSearchQuery(''); }} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            ...
            {/* Review Summary */}
            <View style={[styles.reviewCard, { backgroundColor: colors.bgCardAlt, borderColor: colors.border }]}>
              <Text style={[styles.reviewTitle, { color: colors.textPrimary }]}>📋 Review Your Report</Text>
              
              <View style={styles.reviewRow}>
                <Text style={[styles.reviewLabel, { color: colors.textSecondary }]}>Category:</Text>
                <Text style={[styles.reviewValue, { color: colors.textPrimary }]}>{form.category || 'None'}</Text>
              </View>
              <View style={styles.reviewRow}>
                <Text style={[styles.reviewLabel, { color: colors.textSecondary }]}>Priority:</Text>
                <Text style={[styles.reviewValue, { color: colors.textPrimary }]}>{form.priority}</Text>
              </View>
              <View style={styles.reviewRow}>
                <Text style={[styles.reviewLabel, { color: colors.textSecondary }]}>Title:</Text>
                <Text style={[styles.reviewValue, { color: colors.textPrimary }]} numberOfLines={1}>{form.title || 'None'}</Text>
              </View>
              <View style={styles.reviewRow}>
                <Text style={[styles.reviewLabel, { color: colors.textSecondary }]}>Photo:</Text>
                <Text style={[styles.reviewValue, { color: imageUri ? colors.accent : colors.textMuted }]}>{imageUri ? '✅ Attached' : '❌ None'}</Text>
              </View>
              <View style={styles.reviewRow}>
                <Text style={[styles.reviewLabel, { color: colors.textSecondary }]}>Location:</Text>
                <Text style={[styles.reviewValue, { color: location ? colors.textPrimary : colors.textMuted }]} numberOfLines={2}>
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: rf(13), fontWeight: '700', marginBottom: verticalScale(10), letterSpacing: 0.3 },
  ...
  priorityBtn: {
    ...
    paddingVertical: verticalScale(12), borderRadius: moderateScale(12),
    borderWidth: 2,
  },
  ...
  photoBox: {
    height: verticalScale(160), borderRadius: moderateScale(16),
    borderWidth: 2, borderStyle: 'dashed',
    alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
  },
  photoPreview: { width: '100%', height: '100%' },
  photoBoxText: { fontSize: rf(13), marginTop: verticalScale(8), fontWeight: '500' },
  removePhoto: { position: 'absolute', top: verticalScale(8), right: scale(8), backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 20 },

  locationBtn: {
    flexDirection: 'row', alignItems: 'center', gap: scale(10),
    borderRadius: moderateScale(14), padding: scale(16),
    borderWidth: 1,
  },
  ...
  mapContainer: {
    height: verticalScale(220), borderRadius: moderateScale(16),
  ...
  recenterBtn: {
    position: 'absolute', bottom: verticalScale(12), right: scale(12),
  ...
  addressCard: {
    ...
    borderRadius: moderateScale(14), padding: scale(14),
    borderWidth: 1,
  },
  addressLeft: {
    flex: 1, flexDirection: 'row', alignItems: 'flex-start', gap: scale(10),
  },
