    ...
    return (
      ...
      >
        ...
        <View style={styles.cardBottom}>
          ...
          <Text style={[styles.cardMeta, { color: colors.textMuted }]} numberOfLines={1}>{c.user_name}</Text>
    ...
    >
      <View style={styles.cardTop}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={[styles.cardMeta, { color: colors.textMuted }]}>📍 {cit.barangay || "Unknown"}</Text>
        </View>
      ...
      <View style={[styles.cardBottom, { marginTop: verticalScale(4) }]}>
        <Text style={[styles.cardMeta, { color: colors.statusPending }]}>⏳ {cit.pending} Pending</Text>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bgDark }]} edges={["top"]}>
      <View style={styles.modeToggle}>
        ...
        >
          <Text style={[styles.modeText, { color: colors.textSecondary }, viewMode === "concerns" && { color: "#fff" }]}>
            Concerns
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          <Text style={[styles.modeText, { color: colors.textSecondary }, viewMode === "citizens" && { color: "#fff" }]}>
            Citizens
          </Text>
        </TouchableOpacity>
      </View>
      ...
      {selectedCitizen && viewMode === "concerns" && (
        <View style={[styles.activeFilter, { backgroundColor: colors.accent + "11", borderColor: colors.accent + "33" }]}>
      ...
      {viewMode === "concerns" && (
        <>
          ...
          {showSort && (
            <View style={[styles.sortDropdown, { backgroundColor: colors.bgCard, borderColor: colors.border }]}>
              {SORT_OPTIONS.map((s) => (
                <TouchableOpacity
                  key={s}
                  style={[styles.sortOption, { borderBottomColor: colors.border }, sortBy === s && { backgroundColor: colors.primary + "11" }]}
                  onPress={() => { setSortBy(s); setShowSort(false); }}
                >
                  <Text style={[styles.sortOptionText, { color: colors.textSecondary }, sortBy === s && { color: colors.primary }]}>{s}</Text>
          ...
          <View style={styles.filterRow}>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: scale(8) }}>
              {FILTERS.map((f) => {
                ...
                const count = f === "All" ? concerns.length : concerns.filter((c) => c.status === f).length;
                return (
                  <TouchableOpacity
                    key={f}
                    style={[styles.filterChip, { backgroundColor: colors.bgCard, borderColor: colors.border }, active && { backgroundColor: color + "22", borderColor: color }]}
                    onPress={() => setStatusFilter(f)}
                  >
                    <Text style={[styles.filterText, { color: colors.textMuted }, active && { color }]}>{f}</Text>
                    <Text style={[styles.filterCount, { color: colors.textMuted }, active && { color }]}>{count}</Text>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  statusPill: { paddingHorizontal: scale(8), paddingVertical: verticalScale(3), borderRadius: moderateScale(20) },
  ...
  badge: { paddingHorizontal: scale(8), paddingVertical: verticalScale(2), borderRadius: moderateScale(10) },
