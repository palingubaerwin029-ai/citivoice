...
export default function Users() {
  ...
  useEffect(() => {
    Promise.all([api.get("/users"), api.get("/concerns")])
      .then(([u, c]) => { setUsers(u); setConcerns(c); })
      .catch(console.error)
    ...
    <div className={s.page}>
      ...
      <div className={s.toolbar}>
        <div className={s.search}>
          ...
          {search && <button style={{ background: "none", border: "none", color: "var(--text-3)", cursor: "pointer" }} onClick={() => setSearch("")}>✕</button>}
        </div>
        <select className={s.select} value={brgyFilter} onChange={(e) => setBrgyFilter(e.target.value)}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: selected ? "1fr 320px" : "1fr", gap: 16 }}>
        ...
        <div className={s.tableWrap}>
            ...
            <table className={s.table}>
              ...
              <tbody>
                {filtered.map((u) => {
                  ...
                  return (
                    <tr key={u.id} className={`${s.tr} ${s.trClickable} ${isSel ? s.trSelected : ""}`} onClick={() => setSelected(isSel ? null : u)}>
                      <td className={s.td}>
                      ...
                      <td className={s.td} style={{ fontWeight: 700, color: "var(--text-1)" }}>{count(u.id)}</td>
                      <td className={s.td} style={{ fontWeight: 700, color: "var(--green)" }}>{resCount(u.id)}</td>
                      <td className={s.td} style={{ fontSize: 11, color: "var(--text-3)" }}>{fmtDateShort(u.created_at)}</td>
                      <td className={s.td}><button className={`${s.btn} ${s.btnGhost} ${s.btnSm}`}>{isSel ? "Close" : "View"}</button></td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          {!loading && filtered.length === 0 && <div className={s.empty}><div className={s.emptyIcon}>👤</div><p className={s.emptyTitle}>No citizens found</p></div>}
        </div>
        ...
        {selected && (
          <div className={s.card} style={{ alignSelf: "start" }}>
            <div style={{ padding: "16px 16px 12px", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ width: 52, height: 52, borderRadius: "var(--r-xl)", background: AC(selected.id), display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800, fontSize: 20 }}>{initials(selected.name)}</div>
              <button className={`${s.btn} ${s.btnGhost} ${s.btnSm}`} onClick={() => setSelected(null)}>✕</button>
            </div>
            <div style={{ padding: "0 16px 14px" }}>
              <div style={{ fontSize: 17, fontWeight: 700, color: "var(--text-1)", marginBottom: 4 }}>{selected.name}</div>
              <span className={s.badge} style={{ background: "rgba(59,130,246,0.1)", color: "var(--blue-light)", fontSize: 11 }}>🏙 Citizen</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, padding: "0 16px 14px" }}>
              {[{ l: "Reports", v: count(selected.id), c: "var(--blue-light)" }, { l: "Resolved", v: resCount(selected.id), c: "var(--green)" }].map((x) => (
                <div key={x.l} style={{ background: "var(--surface-3)", borderRadius: "var(--r-md)", padding: 12, textAlign: "center", border: "1px solid var(--border)" }}>
            ...
            <div style={{ padding: "0 16px 16px" }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text-3)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: 10 }}>Recent Concerns</div>
              {concerns.filter((c) => c.user_id === selected.id).slice(0, 5).map((c) => (
                <a key={c.id} href={`/concerns/${c.id}`} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid var(--border)", textDecoration: "none" }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 500, color: "var(--text-1)" }}>{c.title?.slice(0, 35)}…</div>
                    <div style={{ fontSize: 10, color: "var(--text-3)", marginTop: 2 }}>{c.category?.split(" ")[0]}</div>
                  </div>
                  <span className={s.badge} style={{ fontSize: 10, background: (SC[c.status] || "#475569") + "18", color: SC[c.status] || "#94A3B8" }}>{c.status}</span>
                </a>
              ))}
              {count(selected.id) === 0 && <p style={{ color: "var(--text-3)", fontSize: 12, textAlign: "center", padding: 12 }}>No concerns yet</p>}
            </div>
          </div>
        )}
      </div>
