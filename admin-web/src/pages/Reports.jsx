...
export default function Reports() {
  ...
  useEffect(() => {
    Promise.all([api.get("/concerns"), api.get("/users")])
      .then(([c,u]) => { setConcerns(c); setUsers(u); })
      .catch(console.error)
  ...
  const catData=Object.entries(filtered.reduce((a,c)=>{const k=c.category||"Other";a[k]=(a[k]||0)+1;return a;},{})).sort((a,b)=>b[1]-a[1]).map(([name,value])=>({name:name.split(" ")[0],full:name,value}));
  const statusData=Object.entries(SC).map(([name,color])=>({name,value:filtered.filter(c=>c.status===name).length,color}));
  const prioData=[{name:"High",value:filtered.filter(c=>c.priority==="High").length,color:"#EF4444"},{name:"Medium",value:filtered.filter(c=>c.priority==="Medium").length,color:"#F59E0B"},{name:"Low",value:filtered.filter(c=>c.priority==="Low").length,color:"#10B981"}];
  ...
  const brgData=Object.entries(filtered.reduce((a,c)=>{if(c.user_barangay) a[c.user_barangay]=(a[c.user_barangay]||0)+1;return a;},{})).sort((a,b)=>b[1]-a[1]).slice(0,10);
    ...
    <div className={s.page}>
      ...
      <div style={{display:"flex",gap:14,marginBottom:14}}>
        ...
        <div className={s.card} style={{flex:1}}>
          <div className={s.cardHeader}><span className={s.cardTitle}>Status Distribution</span></div>
          <ResponsiveContainer width="100%" height={160}>
            <PieChart><Pie data={statusData} cx="50%" cy="50%" innerRadius={42} outerRadius={68} dataKey="value" paddingAngle={3}>{statusData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip {...TT}/></PieChart>
          </ResponsiveContainer>
          <div style={{padding:"0 16px 14px",display:"flex",flexDirection:"column",gap:6}}>
            {statusData.map((s2)=>(
              <div key={s2.name} style={{display:"flex",alignItems:"center",gap:8,fontSize:12}}>
                <span style={{width:7,height:7,borderRadius:"50%",background:s2.color,flexShrink:0}}/>
                <span style={{color:"var(--text-2)",flex:1}}>{s2.name}</span>
                <span style={{color:s2.color,fontWeight:700}}>{s2.value}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{display:"flex",gap:14,marginBottom:14}}>
        <div className={s.card} style={{flex:2}}>
          <div className={s.cardHeader}><span className={s.cardTitle}>Concerns by Category</span></div>
          <div style={{padding:"12px 12px 8px"}}>
            <ResponsiveContainer width="100%" height={200}>
        ...
        <div className={s.card} style={{flex:1}}>
          <div className={s.cardHeader}><span className={s.cardTitle}>Priority Breakdown</span></div>
          <ResponsiveContainer width="100%" height={160}>
            <PieChart><Pie data={prioData} cx="50%" cy="50%" innerRadius={36} outerRadius={68} dataKey="value" paddingAngle={3} labelLine={false} label={<PriorityLabel/>}>{prioData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip {...TT} formatter={(v,n)=>[`${v} concerns`,`${n} Priority`]}/></PieChart>
          </ResponsiveContainer>
      ...
      <div style={{display:"flex",gap:14,marginBottom:14}}>
        <div className={s.card} style={{flex:1}}>
          <div className={s.cardHeader}><span className={s.cardTitle}>Top Barangays by Reports</span></div>
          <div style={{padding:"8px 16px 16px"}}>
            {brgData.length===0 ? <div style={{padding:40,textAlign:"center",color:"var(--text-3)",fontSize:13}}>No data yet</div> : (
              brgData.map(([brgy,count],i)=>{const pct=total?Math.round(count/total*100):0;return(
        ...
        <div className={s.card} style={{flex:1}}>
          <div className={s.cardHeader}><span className={s.cardTitle}>Most Upvoted Concerns</span></div>
          <div>
            {topUp.length===0 ? <div style={{padding:40,textAlign:"center",color:"var(--text-3)",fontSize:13}}>No data yet</div> : (
              topUp.map((c,i)=>(
                <a key={c.id} href={`/concerns/${c.id}`} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 16px",borderBottom:"1px solid var(--border)",textDecoration:"none"}}>
                  <div style={{width:22,height:22,borderRadius:6,background:"var(--surface-3)",display:"flex",alignItems:"center",justifyContent:"center",color:"var(--text-3)",fontSize:11,fontWeight:700,flexShrink:0}}>#{i+1}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:500,color:"var(--text-1)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.title}</div>
                    <div style={{fontSize:11,color:"var(--text-3)",marginTop:2}}>{c.category?.split(" ")[0]} · {c.user_barangay}</div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
                    <span>👍</span><span style={{color:"#F59E0B",fontWeight:700,fontSize:13}}>{c.upvotes||0}</span>
                  </div>
                </a>
              ))
            )}
      ...
      <div className={s.card}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"20px 24px",flexWrap:"wrap",gap:24}}>
          <div style={{flex:1,minWidth:220}}>
            <h3 style={{color:"var(--text-1)",fontSize:16,fontWeight:700,margin:"0 0 6px"}}>Overall Resolution Performance</h3>
            <p style={{color:"var(--text-3)",fontSize:13,margin:"0 0 16px"}}>{resolved} of {total} concerns resolved</p>
            <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
              {[{l:"Pending",v:pending,c:"#F59E0B"},{l:"In Progress",v:inProgress,c:"#3B82F6"},{l:"Resolved",v:resolved,c:"#10B981"},{l:"Rejected",v:rejected,c:"#EF4444"}].map((x)=>(
                <div key={x.l} style={{display:"flex",alignItems:"center",gap:6}}>
                  <span style={{width:7,height:7,borderRadius:"50%",background:x.c,display:"inline-block"}}/>
                  <span style={{color:"var(--text-3)",fontSize:13}}>{x.l}: </span>
