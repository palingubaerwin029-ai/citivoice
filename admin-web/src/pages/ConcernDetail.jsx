...
export default function ConcernDetail() {
  ...
  useEffect(() => {
    return onSnapshot(doc(db, "concerns", id), (snap) => {
      if (snap.exists()) {
        const d = { id: snap.id, ...snap.data() };
        setConcern(d);
        setSelStatus(d.status || "Pending");
        setNote(d.adminNote || "");
      }
      setLoading(false);
    });
  ...
  const hasChanges =
    selStatus !== concern.status || note !== (concern.adminNote || "");
  const fmt = (ts) =>
    ...
    <div className={s.page}>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          >
            {[
              { icon: "👤", label: "Submitted by", value: concern.userName },
              { icon: "📍", label: "Barangay", value: concern.userBarangay },
              {
                icon: "📅",
                label: "Date Filed",
              ...
              },
              {
                ...
                value: `${concern.upvotes || 0} upvotes`,
              },
            ].map((m, i) => (
              <div
                key={i}
            ...
            <div className={s.card}>
              ...
              >
                ...
                <div>
                  ...
                  >
                    {concern.location.latitude?.toFixed(5)},{" "}
                    {concern.location.longitude?.toFixed(5)}
                  </div>
                </div>
              </div>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {concern.adminNote && (
            ...
            >
              <div className={s.cardHeader}>
                <span className={s.cardTitle} style={{ color: "var(--green)" }}>
                  Previous Response
                </span>
              </div>
              <div
...
function buildTimeline(c) {
  const fmt = (d) =>
    d?.toDate?.()?.toLocaleDateString("en-PH", {
      year: "numeric",
      month: "short",
      day: "numeric",
    }) || "—";
  const steps = [{ event: "Concern submitted", date: fmt(c.createdAt) }];
  if (["In Progress", "Resolved", "Rejected"].includes(c.status))
    steps.push({ event: "Assigned to team", date: fmt(c.updatedAt) });
  if (c.status === "Resolved") {
    steps.push({ event: "Work started", date: fmt(c.updatedAt) });
    steps.push({ event: "Marked as resolved", date: fmt(c.updatedAt) });
  }
  if (c.status === "Rejected")
    steps.push({ event: "Concern rejected", date: fmt(c.updatedAt) });
  return steps;
}
